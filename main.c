...
typedef struct VideoState
{
    ...
    double frame_timer;       //下次回调的计时器
    double frame_last_pts;    //上一帧的pts
    double frame_last_delay;  //上一次增加的delay
    double video_clock;       //下一帧需要播放的时间
    ...
    PacketQueue videoq; //视频队列，存储解完码的信息
...
void video_display(VideoState *is)
{
    ...
    {
        SDL_SetWindowSize(win, screen_width / 2, screen_height / 2);
...
void video_refresh_timer(void *userdata)
{
    ...
    if (is->video_st)
    {
        ...
        else
        {
            ...
            is->video_current_pts = vp->pts; //算出当前展示帧的pts
            is->video_current_pts_time = av_gettime();
            // 当前帧时间减去上一帧的时间，获取两帧间的时差
            ...
            //得到差值决策如何展示， 两帧最少隔0.01 因为一个音频帧是0.01秒
            sync_threshold = (delay > AV_SYNC_THRESHOLD) ? delay : AV_SYNC_THRESHOLD;
            if (fabs(diff) < AV_NOSYNC_THRESHOLD)
            {
                if (diff <= -sync_threshold)
                {
                    fputs("late, show now!", fp);
                    delay = 0;
                }
                else if (diff > sync_threshold)
                {
                    fputs("early, wait to show!", fp);
                    delay = 1.045 * delay;
                }
            }
            else
...
int main(int argc, char *argv[])
{
    ...
    //创建SDLdeWindow
    win = SDL_CreateWindow("gogogo!", 100, 100, 640, 480, SDL_WINDOW_RESIZABLE);
    ...
    //设置一个timer 需要根据视频帧率去计算
    //调函数去渲染
    schedule_refresh(is, 33);

    // 解复用线程， 解packet并进行音视频分类 (in)
